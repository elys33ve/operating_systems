...
#define MAX_READS 10    // MAXIMUM NUMBER OF TIMES THE MESSAGE IS SHOWN EACH READ BEFORE IT STOPS 
...
// deal with the device we are creating.  These four handlers will represent what
...
static int msg_count = 0;   // COUNTER FOR MESSAGES
...
static ssize_t device_read(struct file *flip, char *buffer, size_t len, loff_t *offset) 
    { 
        // FOR TASK 3: THE msg_counter WILL KEEP TRACK OF HOW MANY TIMES THE MESSAGE IS PRINTED.
        // AFTER 10 MESSAGES HAVE BEEN PRINTED, IT RETURNS ZERO TO INDICATE END OF DATA.
        // THE COUNTER IS RESET TO ZERO IN device_open FOR EACH TIME THE DEVICE IS OPENED.

        int bytes_read = 0;

        if (msg_count >= MAX_READS) {      // IF 10 MESSAGES HAVE BEEN SENT, RETURN AND RESET COUNTER
        ...
        msg_count++;        // INCREMENT THE MESSAGE COUNTER
        return bytes_read;
   }
...
// In this case, then, the "write handler" really only need to refuse to do anything
...
static int device_open(struct inode *inode, struct file *file)
   { /* If device is open, return busy */
        ...
        msg_count = 0; // RESET MESSAGE COUNTER ON OPEN
        return 0;
   }
